...
# Get ALL files from stage mapping
all_files = []
for stage, files in stage_mapping.items():
...
# Print results
print("\n" + "="*70)
print("COMPLETE STAGE FIELD UPDATE SUMMARY")
print("="*70 + "\n")
...
for stage, files in stage_mapping.items():
    ...
    for filename in files:
        matching_results = [r for r in results if filename in r]
        if matching_results:
            print(f"  {matching_results[0]}")

print("\n" + "="*70)
print(f"Total files in mapping: {len(all_files)}")
print(f"✅ Successfully updated: {updated_count}")
print(f"⚠️  Already had stage field: {already_updated}")
